...
import { tagsAdapter } from "../../Adapters/post.adapter"
import { getPostTagsService } from "../../Services/post.service"
            ...
            <Link to={`/post`} className="rounded-md border p-5 shadow-md w-full lg:w-8/12 md:w-10/12 bg-white cursor-pointer" onClick={() => window.scrollTo(0, 0)}>
                <div className="flex items-center justify-between space-x-3">
                    <div className="flex">
                        ...
                        <div className="mx-2">
                            <div className="text-md font-bold text-zinc-700">{comment.post_is_anonymous ? 'Anonymous' : comment.user_name}</div>
                            <div className="text-xs font-bold text-zinc-500">{comment.post_is_anonymous ? '' : comment.email}</div>
                        </div>
                    </div>
