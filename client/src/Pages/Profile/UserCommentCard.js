...
const UserCommentCard = ({ commentInfo }) => {
    ...
    useEffect(() => {
        setTimeAgo(elapsedTime(commentInfo.created_at))
    }, [])
    return (
        <div className='flex items-center justify-center my-5'>
            <Link to={`/post/${commentInfo.id_post}`} className="rounded-md p-5 shadow-md w-full bg-zinc-100 dark:bg-zinc-800 cursor-pointer" onClick={() => window.scrollTo(0, 0)}>
                        ...
                        <div className="h-8 w-8 rounded-full">
                            {
                                commentInfo.post_is_anonymous ?
                                    ...
                                    :
                                    <img src={commentInfo.post_user_image} alt="" className='rounded-full' />
                            }


                        </div>
                        <div className="mx-2">
                            <div className="text-md font-bold text-zinc-700 dark:text-zinc-300">{commentInfo.post_is_anonymous ? 'Anonymous' : commentInfo.post_user_name}</div>
                            <div className="text-xs font-bold text-zinc-500">{commentInfo.post_is_anonymous ? '' : commentInfo.post_email}</div>
                        </div>
                    </div>
                ...
                <div className="bg-zinc-200 dark:bg-zinc-700 rounded mt-5 p-4">
                    <div className="mb-6">
                        ...
                        <div className="text-sm text-neutral-600 dark:text-zinc-400 px-4" style={{ wordWrap: "break-word" }}>{commentInfo.content}</div>
                    </div>
